# This file is an adaptation of: https://github.com/Wuyxin/DIR-GNN/blob/main/datasets/mnistsp_dataset.py
...
class MNIST75sp(InMemoryDataset):
    ...
    def __init__(self, root, mode='train', use_mean_px=True,
        ...
        # Load processed file directly
        path = osp.join(self.processed_dir, f"mnist_75sp_{mode}.pt")
    ...
    def download(self):
        for file in self.raw_file_names:
            if not osp.exists(osp.join(self.raw_dir, file)):
                print("Download the files [mnist_75sp_train.pkl], ['mnist_75sp_test.pkl'], and [mnist_75sp_color_noise.pt]" \
                " from https://drive.google.com/drive/folders/1Prc-n9Nr8-5z-xphdRScftKKIxU4Olzh" \
                "and place them in the directory data/MNISTSP/raw/".format(file))
    ...
    def process(self):
        ...
        # Included so the pre-transforms for GTs get executed
