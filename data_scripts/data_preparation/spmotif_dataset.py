# This file is an adaptation of: https://github.com/Wuyxin/DIR-GNN/blob/main/datasets/spmotif_dataset.py
...
class SPMotif(InMemoryDataset):
    ...
    def process(self):
        ...
        for idx, (edge_index, y, ground_truth, z, p) in enumerate(zip(edge_index_list, label_list, ground_truth_list, role_id_list, pos)):
            ...
            roles = torch.as_tensor(np.asarray(z), dtype=torch.long)  # role_id per node
            node_gt = (roles > 0).long()                              # 1 = motif node, 0 = base node
